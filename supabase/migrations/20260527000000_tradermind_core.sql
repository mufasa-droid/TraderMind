...
as $$
begin
  insert into public.users (id, email, full_name, plan, onboarding_completed)
  values (
    new.id,
    new.email,
...
exception
  when others then
    -- Do not block auth.users creation if profile insertion encounters an RLS or constraint error;
    -- the application's /api/auth/init-user endpoint will ensure profile rows are created via service_role.
